...
import { expandQuery, getSuggestions } from "./keyword-expand";
...
export async function smartSearch(
  ...
  // 1) Keyword scoring
  const keywordScored = agents
    .map((agent) => {
      const searchText = getSearchableText(agent);
      const directHits = terms.filter((term) =>
  ...
  let results: SearchResult[] = keywordScored
    .filter((r) => (r._score ?? 0) >= 15)
    .slice(0, maxResults);
...
// Client-side search function (no API needed)
